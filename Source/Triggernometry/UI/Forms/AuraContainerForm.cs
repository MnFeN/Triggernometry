...
namespace Triggernometry.UI.Forms
{
    ...
    public partial class AuraContainerForm : Form
    {
        ...
        internal Core.ActionOld.TextAuraAlignmentEnum TextAlignment { get; set; }
        ...
        internal void AuraActivate()
        {
            ...
            switch (AuraType)
            {
                case AuraTypeEnum.Image:
                default:
                    string img = ctx.EvaluateStringExpression(ctx.Trigger != null ? ctx.Trigger.TriggerContextLogger : (Context.LoggerDelegate)null, ctx.Plugin, PreprocessExpression(ImageExpression));
            ...
            if (ctx.Trigger != null)
            {
                ctx.Trigger.AddToLog(RealPlugin.DebugLevelEnum.Verbose, I18n.Translate("internal/AuraContainer/displayingaura", "Displaying aura window"));
            }
            else
            {
        ...
        internal void AuraDeactivateForm()
        {
            if (ctx.Trigger != null)
            {
                ctx.Trigger.AddToLog(RealPlugin.DebugLevelEnum.Verbose, I18n.Translate("internal/AuraContainer/closingaura", "Closing aura window"));
            }
            else
            {
        ...
        internal void AuraDeactivate()
        {
            if (AuraType == AuraTypeEnum.Image)
            {
                ...
                {
                    if (ctx.Plugin.imageauras.ContainsKey(AuraName) == true)
                    {
                        ctx.Plugin.imageauras.Remove(AuraName);
                    }
                }
            }
            if (AuraType == AuraTypeEnum.Text)
            {
                ...
                {
                    if (ctx.Plugin.textauras.ContainsKey(AuraName) == true)
                    {
        ...
        internal bool UpdateAura(int numTicks)
        {
            ...
            try
            {
                ...
                while (numTicks > 0)
                {
                    ...
                    if (AuraType == AuraTypeEnum.Text)
                    {
                        ...
                        {
                            NewText = ctx.EvaluateStringExpression(ctx.Trigger != null ? ctx.Trigger.TriggerContextLogger : (Context.LoggerDelegate)null, ctx.Plugin, TextExpression);
            ...
            catch (Exception ex)
            {
                if (ctx.Trigger != null)
                {
                    ctx.Trigger.AddToLog(RealPlugin.DebugLevelEnum.Error, I18n.Translate("internal/AuraContainer/updateerror", string.Format("Deactivating aura '{0}' from trigger '{1}' due to update exception: {2}", AuraName, ctx.Trigger.LogName, ex.Message)));
                }
                else
                {
        ...
        internal void PaintText(Graphics gr)
        {
            using (StringFormat sf = new StringFormat())
            {
                ...
                {
                    case Core.ActionOld.TextAuraAlignmentEnum.TopLeft:
                        sf.Alignment = StringAlignment.Near;
                        sf.LineAlignment = StringAlignment.Near;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.TopCenter:
                        sf.Alignment = StringAlignment.Center;
                        sf.LineAlignment = StringAlignment.Near;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.TopRight:
                        sf.Alignment = StringAlignment.Far;
                        sf.LineAlignment = StringAlignment.Near;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.MiddleLeft:
                        sf.Alignment = StringAlignment.Near;
                        sf.LineAlignment = StringAlignment.Center;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.MiddleCenter:
                        sf.Alignment = StringAlignment.Center;
                        sf.LineAlignment = StringAlignment.Center;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.MiddleRight:
                        sf.Alignment = StringAlignment.Far;
                        sf.LineAlignment = StringAlignment.Center;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.BottomLeft:
                        sf.Alignment = StringAlignment.Near;
                        sf.LineAlignment = StringAlignment.Far;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.BottomCenter:
                        sf.Alignment = StringAlignment.Center;
                        sf.LineAlignment = StringAlignment.Far;
                        break;
                    case Core.ActionOld.TextAuraAlignmentEnum.BottomRight:
                        sf.Alignment = StringAlignment.Far;
                        sf.LineAlignment = StringAlignment.Far;
                        break;
                }
